...
def multi_lookup(index, query):
    ...
    for keyword in query:
        if keyword in index:
            for keyval in index[keyword]:
                if keyval[0] in dict:
                    dict[keyval[0]].append(keyval[1])
                else:
    ...
    if len(query) > 1:
        for url in dict:
            is_ok = True
            for e in range(0,len(dict[url])-1):
                if (dict[url][e] +1) != (dict[url][e+1]):
                    is_ok = False
            if(is_ok):
                results.append(url)        
    else :
...
def add_page_to_index(index, url, content):
    ...
    for word in words:
        ...
        add_to_index(index, word, url,indx)
	   
def add_to_index(index, keyword, url,indx):
    if keyword in index:
        index[keyword].append([url,indx])
    else:
        index[keyword] = [[url,indx]]
